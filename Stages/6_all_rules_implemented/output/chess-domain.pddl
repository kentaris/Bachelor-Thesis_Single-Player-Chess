    ...
    (:derived (red_zone ?figure - figure ?kt_file ?kt_rank - location) ;for some figure (capturer) on the board: is there a king of opposite color it can move to?
        (exists(?rook - rook ?cf_file ?cf_rank - location)
            ...
            (and(at ?rook ?cf_file ?cf_rank) ;valid starting location
                ...
                (at ?rook ?kt_file ?kf_rank) ;rook is not at king position
                (not(at ?rook ?kt_file ?kt_rank)) ;rook is where he's supposed to be
                (not(occupied_by_same_color ?rook ?kt_file ?kt_rank)) ;king cannot be checked by his own pieces
                ;can the king be reached by the capturer:
                (or (and(= ?kf_file ?kt_file) ;vertical movement
                        (not(= ?kf_rank ?kt_rank))
                        (vert_reachable ?kf_file ?kf_rank ?kt_file ?kt_rank)
                    )
                    (and(= ?kf_rank ?kt_rank) ;horizontal movement
                        (not(= ?kf_file ?kt_file))
                        (horiz_reachable ?kf_file ?kf_rank ?kt_file ?kt_rank)
                    )
                )
            )
        )
