...
public class Decoder {
    ...
    public static String[] FEN_decodeTo_64String(String FEN, int board_size) {
        ...
        for (char fig : chess.engine.figures.Figures.get_pieces()) {
            ...
            for (int i=0;i<12;i++) {
                maps[i] = builder.toString();
            }
            if (builder.length()!=(board_size*board_size)){
                System.out.println("Decoder: The String builder length is "+builder.length()+" but should be "+(board_size*board_size)+" ("+fig+")");
                System.exit(2);
            }
        }
        return maps;
